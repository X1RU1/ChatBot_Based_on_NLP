...
def handleQuestion(question) -> (str, str):    # Classify the question types
    ...
    result = execute_sparql(matched_entity, matched_relation)  

    if result:  # Factual question
        return "factual", result
    else:
        result = handleEmbedding(matched_entity, matched_relation)
        if result:  # Embedding question
            return "embedding", result
    return None, None

def execute_sparql(entity, relation):
    query = f"""
    SELECT ?x 
    WHERE {{
        <{entity}> <{relation}> ?y.
